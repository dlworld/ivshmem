/*
 * file : ne_ivshmem_shm_host_usr.c
...
int main(int argc, const char **argv)
{
    ...
    ivd.filename = "ivshmem";   /* default shm obj name */
    ivd.filesize = 0x100000;    /* default shm obj size (1MB) */
    ...
    if (ivd.ivshmem_op == NE_IVSHMEM_WRITE) {
        if (optind >= argc) {
            prerr("please specify string to write into %s\n", filename);
            usage(argv[0]);
        }
